...
export default function Projects({ onNavigate }) {
  ...
  const [projects, setProjects] = useState([
    {
      ...
      description: 'إزالة 3 طوابق خرسانية مسلحة مع ترحيل 420 طن أنقاض وتسليم الأرض مستوية وجاهزة للبدء في أعمال الأساسات والخرسانات الجديدة.',
    ...
    },
    {
      ...
      description: 'تفكيك وإزالة الملحق الخارجي والسور والواجهة الجانبية مع المحافظة الكاملة والسلامة الإنشائية للهيكل الرئيسي للفيلا.',
    ...
    },
    {
      ...
      description: 'قشط وتكسير أرضيات البلاط والسيراميك القديم لصالة عرض تجارية وإزالة طبقات الغراء وتجهيز السطح لاستقبال صبة الإيبوكسي.',
    ...
    },
    {
      ...
      description: 'تفريغ قواطع بلوك داخلية وفصل التمديدات الكهربائية والسباكة مع المحافظة التامة على الأعمدة والجسور الخرسانية.',
  ...
  useEffect(() => {
    fetch('/api/projects')
      ...
      .catch(err => console.error('Error fetching projects for gallery:', err))
  ...
  const filteredProjects = filter === 'all' 
    ...
    : projects.filter(p => {
        ...
        if (filter === 'total') return cat.includes('كلي') || cat.includes('هيكل');
        if (filter === 'selective') return cat.includes('جزئي') || cat.includes('توسعة');
        if (filter === 'tiles') return cat.includes('سيراميك') || cat.includes('أرضيات');
        if (filter === 'interior') return cat.includes('ترميم') || cat.includes('تعديل') || cat.includes('جدران');
        return cat === filter;
      });
    ...
    <div className="flex flex-col w-full text-right">
      ...
      <section className="w-full px-gutter-mobile lg:px-gutter-desktop py-space-3xl bg-surface">
        <div className="max-w-[1280px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-space-xl">
          {loading ? (
            <div className="col-span-full py-12 text-center font-headline-sm font-bold text-on-surface-variant">
              جاري تحميل معرض المشاريع الميدانية...
            </div>
              ...
              >
                <div>
                  <div className="relative aspect-[16/10] overflow-hidden bg-surface-container-highest">
                    <img
                      src={project.image || '/images/construction.png'}
                      alt={project.title}
